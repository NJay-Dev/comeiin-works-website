...
function findSupplies(term){
 if(document.body.dataset.product||!$('#catalogue')){location.href='catalogue.html?q='+encodeURIComponent(term)+'#catalogue';return}
 active='all';$('#search').value=term;$('#listing-type').value='all';$('#sort-order').value='review';render();
 $('#catalogue').scrollIntoView({behavior:matchMedia('(prefers-reduced-motion: reduce)').matches?'auto':'smooth'});
 $('#search').focus({preventScroll:true});
}
$('#finder-form')?.addEventListener('submit',event=>{event.preventDefault();findSupplies($('#finder-query').value.trim())});
document.querySelectorAll('[data-suggest]').forEach(button=>button.addEventListener('click',()=>findSupplies(button.dataset.suggest)));
...
document.querySelectorAll('.contact-details .whatsapp').forEach(link=>{
 link.href='https://wa.me/27844106544?text='+encodeURIComponent('Hello Comeiin Works, I would like help with a quotation.');
 link.textContent='WhatsApp: 084 410 6544 ↗';
});
document.querySelectorAll('header nav a[href="#about"],header nav a[href="index.html#about"]').forEach(link=>link.href='about.html');
document.querySelectorAll('footer a[href$="/privacy.html"]').forEach(link=>link.href='privacy.html');

/* Lightweight WhatsApp concierge: a friendly prompt, with no message sent until the visitor continues. */
const whatsappMessage='Hello Comeiin Works, I would like help with a laboratory supply quotation.';
...
function setWhatsAppOpen(open){whatsappChat.hidden=!open;whatsappLaunch.setAttribute('aria-expanded',String(open))}
whatsappWidget.querySelector('.whatsapp-close').addEventListener('click',()=>setWhatsAppOpen(false));
...
if(vantaPage&&!matchMedia('(prefers-reduced-motion: reduce)').matches&&window.VANTA?.NET){
 ...
 }catch(error){console.warn('Animated background unavailable; using the hero image.',error)}
}
